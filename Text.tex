...
\section{$\epsilon_0$-recursion in Peano Arithmetic}

We show that the provably recursive functions of Peano arithmetic are $\epsilon_0$-recursive functions, that is, 
...
\end{center}
Equivalently, $\epsilon_0$ can be defined as the least fixed point of the mapping $\alpha \mapsto \omega^{\alpha}$
where $\alpha$ is an ordinal.

Let us discuss first how one can represent ordinals below $\epsilon_0$.

\subsection{Ordinals below $\epsilon_0$}

Every ordinal $\alpha < \epsilon_0$ is either $0$ or $\alpha$ can be represented uniquely in \emph{Cantor normal form}:
\begin{center}
...
and its predecessor $\alpha - 1$ has the same representation but with $\omega^{\gamma_{k - 1}} \cdot c_{k - 1}$.
Otherwise $\alpha$ is a limit ordinal ($\operatorname{Lim}(\alpha)$) and it has infinitely many possible
...
\begin{definition}
  Let $\alpha < \epsilon_0$ and $n < \omega$, define a finite set of ordinals $\alpha[n]$ the following way:
...
\begin{col}
  Let $\alpha < \epsilon_0$ be a limit ordinal, then for every $0 \neq n < \omega$ $\alpha(n) \in \alpha[n + 1]$.
...
\begin{lemma}
  If $\beta < \alpha$ and the maximum coefficitent of $\beta$ is $\leq n$, so $\beta \in \alpha[n]$. 
\end{lemma}
...
\begin{lemma} If $\alpha = \delta + \omega^{\gamma}$, then for all $n < \omega$
  ...
  \end{center}
  Thus for each $\alpha < \epsilon_0$, $G_{\alpha}(n)$ is the elementary function
